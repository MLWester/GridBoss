...
function SectionCard({ children }: { children: React.ReactNode }): ReactElement {
  return <div className="rounded-2xl border border-slate-800/70 bg-slate-900/60 p-5 shadow shadow-slate-950/30">{children}</div>
}
...
export function LeagueOverviewPage(): ReactElement {
  const { overview, isLoading, error, refetch } = useOutletContext<LeagueOutletContext>()
  ...
  const podium = useMemo(() => recentResult?.podium ?? [], [recentResult?.podium])
    ...
    return (
      <SectionCard>
        <p className="text-sm text-rose-100">
          We could not load the league overview. <span className="text-xs text-rose-300">{error.message}</span>
        </p>
        <button
          type="button"
  ...
  return (
    <div className="grid gap-6 lg:grid-cols-[2fr,1fr]">
      <div className="space-y-4">
        <SectionCard>
          <p className="text-xs font-semibold uppercase tracking-wide text-slate-400">Next event</p>
          {nextEvent ? (
            <div>
              <p className="mt-2 text-lg font-semibold text-slate-100">{nextEvent.name}</p>
              <p className="text-sm text-slate-400">{nextEvent.track}</p>
              <p className="mt-3 text-xs text-slate-300">Lights out: {formatDateTime(nextEvent.startTime)}</p>
            ...
            <p className="mt-2 text-sm text-slate-400">No upcoming event is scheduled yet.</p>
          )}
        </SectionCard>

        <SectionCard>
          <p className="text-xs font-semibold uppercase tracking-wide text-slate-400">Last result</p>
          {recentResult ? (
            <div>
              <p className="mt-2 text-lg font-semibold text-slate-100">{recentResult.event.name}</p>
              <p className="text-sm text-slate-400">{recentResult.event.track}</p>
              <p className="mt-3 text-xs text-slate-300">Completed: {formatDateTime(recentResult.event.startTime)}</p>
              {podium.length > 0 ? (
                <ul className="mt-4 space-y-2 text-sm text-slate-200">
                  {podium.slice(0, 3).map((entry) => (
                    <li key={entry.position} className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="font-semibold text-slate-100">{entry.driverName}</p>
                        {entry.teamName ? <p className="text-xs text-slate-400">{entry.teamName}</p> : null}
                      </div>
                      {entry.points != null ? (
                        <span className="ml-auto text-xs text-slate-300">{entry.points} pts</span>
                      ) : null}
                    </li>
                  ))}
                </ul>
                ...
                <p className="mt-3 text-sm text-slate-400">
                  Podium details will populate here once the steward posts official results.
                </p>
              )}
            </div>
          ) : (
            <p className="mt-2 text-sm text-slate-400">Race results will appear here once events are completed.</p>
          )}
        </SectionCard>
      </div>
      ...
      <div className="space-y-4">
        <SectionCard>
          <p className="text-xs font-semibold uppercase tracking-wide text-slate-400">Discord integration</p>
          {overview?.discordLinked ? (
            <p className="mt-3 text-sm text-emerald-200">
              Discord is linked. Announcements and race control messages are ready to go.
            </p>
          ) : showDiscordCta ? (
            <div className="mt-3 space-y-3 text-sm text-slate-300">
              <p>Connect your Discord server to send race summaries and schedule notifications.</p>
              <button
                type="button"
            ...
            <div className="mt-3 space-y-3 text-sm text-slate-300">
              <p>
                Discord linking unlocks on the Pro plan. Upgrade when billing launches to automate results recaps and race
                announcements.
              </p>
              <p className="text-xs text-slate-500">Current plan: {planLabel}</p>
            </div>
          )}
        </SectionCard>
